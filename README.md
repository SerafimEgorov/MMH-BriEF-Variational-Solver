...
Please cite the accompanying paper for this code in any work that is using it:
...
First, please, **[read INSTALL.md](INSTALL.md)** and when the environment installation is completed, continue from here. 
...
- **`-f`, `--field`** — Basename of the random field (without `_f.npz`). The solver reads `{fpath}/{f}_f.npz` and the precomputed integrated field `{fpath}/{f}_F_N{N}pts.npz`.
